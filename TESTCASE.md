...
Version: 1.0-draft
Based on: `CLASS.md` / `SPEC.md` v1.0-draft / `SEQUENCE.md` / `USECASE.md`
Primary target: Phase 1 dry-run, Phase 3 AgentInvoker, Phase 4 summary analyzer
...
| ID | 条件 | 期待結果 | テスト方法 |
|---|---|---|---|
| TC-ANA-001 | finished複数試合 | 総数・有効数と陣営別勝敗を集計 | `test_analyze_results.py` で自動テスト済み |
| TC-ANA-002 | プレイヤー別データ | 勝率、役職別勝率、処刑数が正しい | `test_analyze_results.py` で自動テスト済み |
| TC-ANA-003 | 役職別データ | 割当数、勝利数、勝率が正しい | `test_analyze_results.py` で自動テスト済み |
| TC-ANA-004 | phase != finished | 警告付きでスキップ | `test_analyze_results.py` で自動テスト済み |
| TC-ANA-005 | JSON破損 | 警告付きでスキップ | `test_analyze_results.py` で自動テスト済み |
| TC-ANA-006 | 必須キー欠落 | 警告付きでスキップ | `test_analyze_results.py` で自動テスト済み |
| TC-ANA-007 | logs-root不存在 | 非0終了 | `test_analyze_results.py` で自動テスト済み |
| TC-ANA-008 | gameディレクトリなし | 有効0、警告付きで正常終了 | `test_analyze_results.py` で自動テスト済み |
| TC-ANA-009 | --format markdown | Markdownを標準出力 | `test_analyze_results.py` で自動テスト済み ＆ 手動確認済み |
| TC-ANA-010 | --format json | 4トップレベルキーのJSONを標準出力 | `test_analyze_results.py` で自動テスト済み ＆ 手動確認済み |
| TC-ANA-011 | --output | 親ディレクトリを作成してファイル保存 | `test_analyze_results.py` で自動テスト済み ＆ 手動確認済み |
| TC-ANA-012 | public_log.md 等の欠落 | 試合は有効。集計対象外のため影響なし | `game_state.json` のみを主入力とするため影響がないことを手動確認済み |
